    ...
    <>
      <Container>
        ...
        {sellsState.map((el, idx) => {
          const correspondingAsset = allAssetsState.find(
            (all) => all.idx === el.assetIdx,
          );
          let id = 0;
          if (correspondingAsset) {
            return (
              <>
                {el.price !== "0" && (
                  <Card
                    key={correspondingAsset.idx}
                    style={{ width: "18rem", margin: "20px" }}
                  >
                    <Card.Text>ID лота: {idx}</Card.Text>
                    <Card.Img
                      variant="top"
                      src={`assets/${correspondingAsset.img}`}
                    />
                    <Card.Body>
                      ...
                      <Card.Text>
                        Кол-во токенов: {correspondingAsset.releasedAmount}
                      </Card.Text>
                      <Card.Text>
                        Время создания:{" "}
                        {new Date(
                          correspondingAsset.dateCreate * 1000,
                        ).toISOString()}
                      </Card.Text>
                    </Card.Body>
